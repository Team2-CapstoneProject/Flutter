import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

String baseUrl = 'https://fullstak-vila-reservation.vercel.app/api';

// Color
Color primaryColor = const Color(0xFF1D3557);
Color whiteColor = const Color(0xffFFFFFF);
Color white70Color = const Color(0xFFD9D9D9);
Color blueColor = const Color(0xff14193F);
Color greyColor = const Color(0xFF525252);
Color grey2Color = const Color(0xFF515151);
Color grey95 = const Color(0xFFF2F2F2);
Color darkGrey = const Color(0xFF292D32);
Color cyanBlue = const Color(0xFFDBEAFF);
Color darkerGrey = const Color(0xFF484C52);
Color redCandy = const Color(0xFFFF3055);
Color blackColor = const Color(0xFF000000);
Color blueBlack = const Color(0xFF070A13);
Color grey100 = const Color(0xFF64748B);
Color yellowColor = const Color(0xFFFFC700);
Color grey3Color = const Color(0xFF64748B);
Color redColor = const Color(0xFFC51414);
Color greenColor = const Color(0xFF2CF564);

// TextStyle
TextStyle primaryTextStyle = GoogleFonts.poppins(color: primaryColor);
TextStyle whiteTextStyle = GoogleFonts.poppins(color: whiteColor);
TextStyle greyTextStyle = GoogleFonts.poppins(color: greyColor);
TextStyle grey2TextStyle = GoogleFonts.poppins(color: grey2Color);
TextStyle blueTextStyle = GoogleFonts.poppins(color: blueColor);
TextStyle blackTextStyle = GoogleFonts.poppins(color: blackColor);
TextStyle redCandyTextStyle = GoogleFonts.poppins(color: redCandy);
TextStyle blueBlackTextStyle = GoogleFonts.poppins(color: blueBlack);
TextStyle grey100kTextStyle = GoogleFonts.poppins(color: grey100);
TextStyle grey3TextStyle = GoogleFonts.poppins(color: grey3Color);
TextStyle darkGreyTextStyle = GoogleFonts.poppins(color: darkGrey);
TextStyle greenTextStyle = GoogleFonts.poppins(color: greenColor);

// FontWeight
FontWeight thin = FontWeight.w100;
FontWeight extraLight = FontWeight.w200;
FontWeight light = FontWeight.w300;
FontWeight regular = FontWeight.w400;
FontWeight medium = FontWeight.w500;
FontWeight semiBold = FontWeight.w600;
FontWeight bold = FontWeight.w700;
FontWeight extraBold = FontWeight.w800;
FontWeight black = FontWeight.w900;
